...
typedef struct RecordingConfig {
     ...
     * 
     * For more information, see [Use Cloud Proxy](https://docs-preview.agoralab.co/en/Recording/cloudproxy_recording?platform=All%20Platforms)。
     */
    ...
    /** Sets whether to support h.265 decode capbility
     * - true: Record Sdk could recv h.265 video stream if remote user send
     * - false: (Default) Record Sdk could not recv h.265 video stream, and other users within the channel are no longer sending h.265 streams
     */
...
public:
     ...
     *               Pointer to an array of WatermarkConfig.
     * 
     * @return
     * - 0: Success.
     * - < 0: Failure.
